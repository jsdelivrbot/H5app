...
(function (exports) {
    ...
    PieChart.prototype._animateDraw = function(drawFunc) {
        ...
        var dr = setInterval(function() {
            ...
            ctx.clearRect(0, 0, 600, 600);
            drawFunc.call(self, startDeg, totalDeg);
            startDeg += incre;
            totalDeg += incre;
            ...
            spanTime = (spanTime >= 60) ? spanTime : (spanTime + 1);
    ...
    RingChart.prototype._drawRingChart = function(startDeg, totalDeg) {
        var ctx = this.ctx;
        // var startDeg = -90;
        startDeg = 30;
        var deg = 0;
        ...
        for (key in this.data.percentage) {
            this.data.info[key] = {};
            deg = this.data.percentage[key] * totalDeg;
            if (deg === 0) {
                continue;
            }
            endDeg = startDeg + deg;
            ...
            ctx.stroke();
            ctx.closePath();
            if (Math.abs(endDeg - startDeg) > 10) {
                var labelImg = this.data.sorted[key].img;
                var labelX = this.cx + (this.r) * Math.cos(utils.getRadius(startDeg + (endDeg - startDeg) / 2)) - 52 / 2;
                var labelY = this.cy + (this.r) *  Math.sin(utils.getRadius(startDeg + (endDeg - startDeg) / 2)) - 52 / 2;
                ctx.drawImage(labelImg, labelX, labelY, 52, 52);
            }
            // next sector data
            startDeg = endDeg;
            startPos.x = endPos.x;
    ...
    RChart.prototype.getWrapperSize = function() {
    ...
    // draw canvas
    RChart.prototype.draw = function(opt) {
        var obj;
        switch (this.type) {
            case 'ringchart':
