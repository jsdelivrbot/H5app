...
function outputHtml(path) {
    ...
    // 不同环境使用不同的api接口
        ...
        // view视图路径替换成对应的文件内容
        .pipe(gulpif('*.js', replace(/require\s*\(\s*[\'|\"]([\w+|\\|\/]+\.view)[\'|\"]\s*\)/g, function(match, $1) {
        ...
        .pipe(gulpif('*.html', replace(/(href=|src=)\"(.+\.css|.+\.js)\"/g, function(match, $1, $2) {
            return $1 + '"/' + config.project + '/' + $2 + '?_=' + timestamp + '"';
        })))
...
function outputImages() {
    ...
    imgPaths.forEach(function(path, index) {
        ...
        // 3. 未压缩过的图片(增量压缩)
        // 4. 只在dev环境进行图片压缩
        if (/(\.png|\.jpg|.jpeg)$/.test(path) && path.indexOf('/notMin/') == -1 && !tinypngCacheData[path] && cmd == 'dev') {
            gulp.src(path)
                .pipe(plumber({
    ...
    connect.server({
        ...
        middleware: function() {
            return [function(req, res, next) {
                ...
                gulp.src(url)
                    .pipe(gulpif('*.js', replace(/\{\{(\w+Api)\}\}/g, function(match, $1) {
                        var envArr = ['serve', 'dev', 'test', 'prod', 'beta'];
                        //var env = cmd == 'beta' ? 'prod' : cmd;
                        var env = cmd;
...
gulp.task('test', function() {
    runSequence('clean', 'replace:test', ['handle', 'data']);
});
